...
label_dict = {
    'aoyama': 0, 'keio': 1, 'phoenix': 2, 'seikei': 3, 'sophia': 4
}


def load_data(data_type):
    filenames, images, labels = [], [], []
    path = '{}/{}'.format('modeldata', data_type)
    walk = filter(lambda _: not len(_[1]) and data_type in _[0], os.walk(path))
...
def main():
    with tf.Graph().as_default():
        ...
        y_ = tf.placeholder('float', shape=[None, 5])  # 10 classes
...
def doclassify():
    with tf.Graph().as_default():
        ...
        print(y_conv.eval(feed_dict={x: [test_images[0]], keep_prob: 1.0}))
        print(np.argmax(y_conv.eval(feed_dict={x: [test_images[0]], keep_prob: 1.0})))
